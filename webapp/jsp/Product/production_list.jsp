...
<head>
  ...
  <style>
    ...
    h1 { text-align: center; margin: 0 0 18px; letter-spacing: .02em; }

    .controls { display:flex; align-items:center; gap:12px; margin-bottom:14px; }
    .controls .search-form { display:flex; gap:8px; flex:1; }
    .search-input {
      flex:1; padding:10px 12px; border:1px solid #d8dde3; border-radius:8px; font-size:14px;
    }
    .btn {
      padding:10px 14px; border:0; border-radius:8px; cursor:pointer; font-size:14px; font-weight:600;
      transition: transform .05s ease, box-shadow .2s ease, opacity .2s ease;
    }
    .btn:active { transform: translateY(1px); }
    .btn-primary { background:#1677ff; color:#fff; box-shadow:0 2px 8px rgba(22,119,255,.25); }
    .btn-success { background:#22c55e; color:#fff; box-shadow:0 2px 8px rgba(34,197,94,.25); }
    .btn-primary:hover, .btn-success:hover { opacity:.95; }

    /* 표 래퍼(가로 스크롤) */
    .table-responsive { overflow-x:auto; border:1px solid #eef1f4; border-radius:10px; }

    table {
      width:100%; min-width:980px; border-collapse:separate; border-spacing:0; table-layout:fixed;
    }
    thead th {
      position:sticky; top:0; z-index:1; background:#f6f8fb; color:#344053; font-weight:700;
      border-bottom:1px solid #e7ecf1; padding:12px 10px;
    }
    tbody td {
      padding:12px 10px; border-bottom:1px solid #f0f3f6; color:#222;
      white-space:nowrap; overflow:hidden; text-overflow:ellipsis;
    }
    tbody tr:hover { background:#fafcff; }

    /* 가독성 너비 가이드 */
    th:nth-child(1), td:nth-child(1) { width:110px; } /* 생산번호 */
    th:nth-child(2), td:nth-child(2) { width:100px; } /* 제품코드 */
    th:nth-child(3), td:nth-child(3) { width:140px; } /* 제품명 */
    th:nth-child(4), td:nth-child(4) { width:120px; } /* 생산 시작일 */
    th:nth-child(5), td:nth-child(5),
    th:nth-child(6), td:nth-child(6) { width:110px; } /* 목표/완료량 */
    th:nth-child(7), td:nth-child(7) { width:90px; }  /* 담당자 */
    th:nth-child(8), td:nth-child(8) { width:140px; } /* 관리 */

    .action-links { white-space:nowrap; }
    .action-links a { color:#1677ff; text-decoration:none; margin-right:8px; }
    .action-links a:hover { text-decoration:underline; }

    /* 차트 */
    .chart-container { margin:18px auto 4px; width:100%; max-width:900px; height:320px; }
  ...
  <div class="main-container">
    ...
    <div class="content-area">
      <div class="container">
        ...
        <!-- 검색 & 등록 -->
        <div class="controls">
          <form class="search-form" action="${ctx}/productionList" method="get">
            <input type="text" class="search-input" name="q" placeholder="생산번호, 제품코드, 제품명 검색" value="${param.q}" />
            ...
            <tbody>
              <c:forEach var="item" items="${productionList}">
                <tr>
                  ...
                  <td>
                    <a href="#" class="js-prod-detail" data-no="${item.productionNo}">
                      ${item.stName}
                    </a>
                  </td>
                  <td><fmt:formatDate value="${item.prStart}" pattern="yyyy-MM-dd"/></td>
                  <td>${item.prTarget}</td>
                  <td>${item.prCompleted}</td>
                  <td>${item.employeeNo}</td>
                  <td class="action-links">
                    <a href="${ctx}/production/update?no=${item.productionNo}">수정</a>
                    <a href="${ctx}/production/delete?no=${item.productionNo}" onclick="return confirm('삭제하시겠습니까?');">삭제</a>
                  </td>
                </tr>
              </c:forEach>
            </tbody>
