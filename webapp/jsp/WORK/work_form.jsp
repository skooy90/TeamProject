  ...
  <div class="main-container">
    ...
    <div class="content-area">
      <div class="form-card">
        ...
        <form method="post" action="${ctx}${isEdit ? '/work/update' : '/work/insert'}">
          <div class="form-grid">
            ...
            <c:choose>
              <c:when test="${isEdit}">
                ...
                <input type="hidden" id="productionNo" name="productionNo" value="${work.productionNo}" />
              </c:when>

              <c:otherwise>
                <c:choose>
                  <c:when test="${empty productions}">
                    <div style="grid-column:1 / -1; color:#64748b; font-size:13px;">
                      등록 가능한 생산이 없습니다. 먼저 생산을 등록해 주세요.
                    </div>
                  </c:when>
                  ...
                  <c:otherwise>
                    <select id="productionNo" name="productionNo" required>
                      ...
                      <c:forEach var="p" items="${productions}">
                        <option
                          value="${p.productionNo}"
                          data-standard="${p.standardCode}"
                          data-name="${p.stName}"
                          data-target="${p.prTarget}">
                          ${p.productionNo} — ${p.stName} (${p.standardCode})
                        </option>
                      </c:forEach>
                    </select>
                  </c:otherwise>
            ...
            <input id="productName" type="text" readonly
                   ...
                   placeholder="생산번호 선택 시 자동표시" />

            <input type="hidden" id="standardCode" name="standardCode"
                   value="${isEdit ? work.standardCode : ''}" />

            <label>담당자 사번</label>
            <c:choose>
              <c:when test="${isEdit}">
                <input type="text" value="${work.employeeNo}" readonly />
                <input type="hidden" name="employeeNo" value="${work.employeeNo}" />
              </c:when>
              <c:otherwise>
            ...
            <label for="woSchedule">작업 일정</label>
            <input id="woSchedule" name="woSchedule" type="date"
                   value="<fmt:formatDate value='${work.woSchedule}' pattern='yyyy-MM-dd'/>" />

            <label for="woQuantity">작업량</label>
            <input id="woQuantity" name="woQuantity"
                   value="${empty targetFromServer ? 0 : targetFromServer}" readonly />

            <label for="woCompleted">작업 완료량</label>
            <input id="woCompleted" name="woCompleted" value="${isEdit ? work.woCompleted : 0}" />
          </div>
