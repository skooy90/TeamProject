...
public class MatDAO {
    ...
    // R - 제품 유형별 재고 조회
    public List<MaterialDTO> selectMaterialsByType(String type) {
        List<MaterialDTO> materialList = new ArrayList<>();
        String sql = "SELECT m.MATERIAL_CODE, m.STANDARD_CODE, s.ST_NAME, s.ST_TYPE, s.ST_UNIT, " +
                    "m.MA_QUANTITY, m.EMPLOYEE_NO, u.US_NAME, u.US_POSITION, " +
                    "m.MA_CREATION_DATE, m.MA_UPDATE_DATE " +
    ...
    // R - 담당자별 재고 조회
    public List<MaterialDTO> selectMaterialsByEmployee(String employeeNo) {
        List<MaterialDTO> materialList = new ArrayList<>();
        String sql = "SELECT m.MATERIAL_CODE, m.STANDARD_CODE, s.ST_NAME, s.ST_TYPE, s.ST_UNIT, " +
                    "m.MA_QUANTITY, m.EMPLOYEE_NO, u.US_NAME, u.US_POSITION, " +
                    "m.MA_CREATION_DATE, m.MA_UPDATE_DATE " +
    ...
    // 검색 메서드
    public List<MaterialDTO> searchMaterials(String searchType, String searchKeyword) {
        List<MaterialDTO> materialList = new ArrayList<>();
        String sql;
    ...
    // 전체 검색 메서드 (재고코드, 제품코드, 제품명 모두 검색)
    public List<MaterialDTO> searchAllFields(String searchKeyword) {
        List<MaterialDTO> materialList = new ArrayList<>();
        String sql = "SELECT m.MATERIAL_CODE, m.STANDARD_CODE, s.ST_NAME, s.ST_TYPE, s.ST_UNIT, " +
                    "m.MA_QUANTITY, m.EMPLOYEE_NO, u.US_NAME, u.US_POSITION, " +
                    "m.MA_CREATION_DATE, m.MA_UPDATE_DATE " +
        ...
        try {
            ...
            pstmt.setString(1, keyword);  // 재고코드
